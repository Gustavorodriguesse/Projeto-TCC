...
CREATE TABLE IF NOT EXISTS visitantes (
    ...
    data_hora_entrada TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS rotas_maritimas (
    ...
    distancia_km DOUBLE PRECISION NOT NULL CHECK (distancia_km > 0),
...
CREATE TABLE IF NOT EXISTS navios (
    ...
    tempo_fora_porto_segundos BIGINT DEFAULT 0 CHECK (tempo_fora_porto_segundos >= 0), -- RF 5
    porto_origem TEXT,
    porto_destino TEXT,
    data_ultima_manutencao TIMESTAMPTZ
);
...
CREATE TABLE IF NOT EXISTS checklist_modelos (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tipo_carga_id BIGINT NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS checklist_itens (
    ...
    is_critico BOOLEAN NOT NULL DEFAULT FALSE, -- TRUE = Crítico, FALSE = Não crítico
...
CREATE TABLE IF NOT EXISTS conteineres (
    ...
    tipo_carga_id BIGINT, -- Um contêiner transporta apenas um tipo de carga (RN 5)
    ...
    navio_id BIGINT, -- Alocado a um navio (RN 6: navio carrega apenas contêineres)
...
CREATE TABLE IF NOT EXISTS cargas (
    ...
    peso_kg DOUBLE PRECISION NOT NULL CHECK (peso_kg > 0), -- Atributo obrigatório (RN 13)
    volume_m3 DOUBLE PRECISION NOT NULL CHECK (volume_m3 > 0), -- Atributo obrigatório (RN 13)
    valor_declarado DOUBLE PRECISION NOT NULL CHECK (valor_declarado >= 0), -- Atributo obrigatório (RN 13)
    natureza TEXT NOT NULL, -- Atributo obrigatório (RN 13)
    data_prevista_agendamento TIMESTAMPTZ NOT NULL,
    data_entrada_porto TIMESTAMPTZ,
    data_saida_porto TIMESTAMPTZ,
    ...
    conteiner_id BIGINT, -- Vinculada ao contêiner
...
CREATE TABLE IF NOT EXISTS inspecoes_carga (
    ...
    data_inspecao TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS respostas_checklist (
    ...
    conforme BOOLEAN NOT NULL, -- TRUE = Conforme, FALSE = Não Conforme
    observacao TEXT,
...
CREATE TABLE IF NOT EXISTS movimentacoes_estivador (
    ...
    data_hora_registro TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (carga_id) REFERENCES cargas(id),
...
CREATE TABLE IF NOT EXISTS historico_manutencoes (
    ...
    entidade_id BIGINT NOT NULL, -- ID da entidade correspondente
...
-- Registro imutável de decisões de alto impacto tomadas no sistema.
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS trail_decisoes (
    ...
    imutavel BOOLEAN NOT NULL DEFAULT TRUE, -- Registro imutável (RF 13)
    FOREIGN KEY (usuario_codigo) REFERENCES funcionarios(codigo_individual)
);
