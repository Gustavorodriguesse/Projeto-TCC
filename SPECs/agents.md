...
| 2 (plano) | `SPECs/tasks.md` | Decomposição em 10 fases (T1.1 → T10.10) com dependências críticas de execução. **Siga esta ordem.** |
...
- **Front-end:** Next.js (React) + TypeScript + **Tailwind CSS** (a identidade visual do tema já é Tailwind — porte os tokens para o `tailwind.config`).
- **Banco de dados e back-end: Supabase (obrigatório).**
  ...
  - **Row Level Security (RLS)** habilitado em **todas** as tabelas — as 3 camadas de visão (seção 5) devem ser implementadas como políticas RLS, não apenas no front-end.
  - **Supabase Auth** para sessão (ver modelo de login na seção 6).
  - **Supabase Storage** para PDFs gerados (relatórios A4 e etiquetas), se aplicável.
  - Variáveis `NEXT_PUBLIC_SUPABASE_URL` e `NEXT_PUBLIC_SUPABASE_ANON_KEY` em `.env.local` (nunca comitar segredos).
...
- **PDF:** geração no formato A4 (relatórios) e etiquetas 10×10/10×15 cm — pode ser biblioteca JS (ex.: `pdfmake`, `@react-pdf/renderer`) ou rota server-side.
- **Gráficos dos dashboards do Diretor:** biblioteca leve (ex.: Recharts).
...
Resumo das 10 fases:
1. **F1 — Fundação & Modelagem:** MER, DDL no Supabase, seed (cargos, tipos de carga, usuários de teste), estratégia do código individual.
2. **F2 — Autenticação & RBAC:** login por código individual, cargo travado para confirmação, guard de autenticação, 3 camadas de visão, controle por rota/API.
3. **F3 — Cadastros base (CRUDs):** funcionários, visitantes, tipos de carga, rotas, navios, contêineres, guindastes + atualizações operacionais.
4. **F4 — Fluxo de Cargas (core):** agendamento → recebimento → inspeção/checklist → armazenagem → vinculação → pronta p/ entrega → liberação → trânsito → entregue (+ cancelamento).
5. **F5 — Manutenções & Emergências:** estados de reforma, solicitação/aprovação, histórico, preventiva 3 anos, alarmes.
6. **F6 — QR Code & Etiquetas:** geração única, impressão PDF de etiqueta, reimpressão com log, leitura por câmera com redirecionamento por cargo.
7. **F7 — Dashboards, Pesquisa & Relatórios:** cards operacionais, dashboards do Diretor com gráficos, pesquisa com os 5 filtros exatos, PDF A4 em 4 seções, relatório de produtividade.
8. **F8 — Logs, Trail & Delegação:** log de alterações, trail imutável com retificação, delegação de supervisor.
9. **F9 — Localização & Tempos:** coordenadas fictícias, classificação de localização, cálculos de tempo e estimativa (distância ÷ 33 km/h).
10. **F10 — Testes & Implantação:** unitários das regras de negócio, integração do fluxo completo, segurança (RLS), QR, PDF.
...
- Perda/esquecimento (RN 15 / T2.8): o **Técnico em Portos** invalida o código e gera um novo vinculado à mesma matrícula.
...
- [ ] Código em TypeScript, sem `any` desnecessário, lint passando.
...
- [ ] Nenhum segredo comitado (apenas `.env.example`).
- [ ] Commit messages claros referenciando o código da tarefa (ex.: `T4.16 — liberação de navio com propagação para cargas vinculadas`).
