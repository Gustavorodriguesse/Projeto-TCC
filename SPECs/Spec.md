...
7. **Inspetor:** coordenar procedimentos de emergência (acionamento de alarmes); cadastrar novos navios, novos contêineres e novos guindastes; realizar inspeção técnica formal das cargas com preenchimento do checklist e decisão de aprovação ou recusa; acessar todas as funcionalidades dos cargos operacionais.
...
### 2. Cadastro e Gestão de Navios, Contêineres, Cargas, Rotas, Tipos de Carga e Guindastes

O sistema deve permitir o registro e consulta das seguintes entidades:

- **Navios:** nome, número IMO, data de registro no sistema, quantidade de cargas realizadas, estado operacional, coordenadas de localização, tempo fora do porto, porto de origem e porto de destino da viagem. O cadastro inicial do navio (nome, IMO, porto de origem e destino) é feito pelo Inspetor; a atualização dos dados operacionais (estado, coordenadas, tempo fora do porto) é feita pelo Planejador de Pátio e de Navios.
- **Contêineres:** número de identificação, tipo de carga vinculada, material carregado, data de fabricação, data da última manutenção, tempo de uso, estado, navio vinculado. O cadastro inicial do contêiner é feito pelo Inspetor; a atualização dos dados operacionais é feita pelo Planejador de Pátio e de Navios.
...
- **Guindastes:** número de identificação, estado (operante, em manutenção), data da última manutenção. O cadastro do guindaste é feito pelo Inspetor; a solicitação de manutenção é feita pelo Supervisor, seguindo o mesmo fluxo de aprovação de manutenção aplicado a navios.

O sistema deve manter **histórico de manutenções** para navios, contêineres e guindastes, contendo data e descrição dos serviços realizados, inseridos manualmente pelo responsável.
...
O sistema deve permitir que um Supervisor oficial designe um substituto temporário com os mesmos poderes de liberação, registrando o período de vigência da delegação. Cada Supervisor pode ter apenas um substituto ativo por vez. O Supervisor titular pode revogar a delegação a qualquer momento antes do fim do período de vigência registrado, encerrando os poderes do substituto imediatamente.
...
9. O tempo de entrega da carga é calculado pela distância da rota pré-cadastrada dividido pela velocidade média de **33 km/h**. O sistema calcula automaticamente a estimativa de chegada. Caso não exista rota cadastrada entre o porto de origem e o destino informado, o sistema bloqueia a liberação do navio para saída até que o Supervisor cadastre a rota correspondente.
...
12. Se uma carga está dentro de um contêiner e o contêiner está no navio, uma atualização de **estado operacional ou localização** do navio ou do contêiner reflete automaticamente nas cargas vinculadas (ex: navio muda para `FORA_DO_PORTO` → todas as cargas vinculadas passam a refletir esse deslocamento). Alterações de nome ou de dados cadastrais afetam apenas o próprio item, sem propagação. Quando um contêiner ou navio está em manutenção, não haverá cargas dentro dele.
...
15. Todo acesso ao sistema exige o código individual do funcionário, vinculado ao seu cargo, garantindo a identificação nas operações e logs. Em caso de perda ou esquecimento do código, o Técnico em Portos é responsável por invalidar o código anterior e gerar um novo, vinculado à mesma matrícula.
...
13. O sistema não controla lotação máxima, capacidade de peso ou quantidade de contêineres por navio, nem a capacidade física de vagas/berços do pátio para atracação simultânea de navios.
